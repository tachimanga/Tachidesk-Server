...
import app.tachimanga.cloud.model.dto.manga.ExtensionDTO
...
fun ExtensionTable.toSyncData(entry: ResultRow): SyncCommitDTO {
    val sync = SyncCommitDTO()
    sync.clientDataId = entry[id].value
    sync.dataType = DataType.Extension.name
    val extension = ExtensionDTO()

    extension.clientCreatedAt = entry[createAt]
    extension.clientUpdatedAt = entry[updateAt]
    extension.clientDeleted = false

    extension.pkgName = entry[pkgName]
    extension.versionName = entry[versionName]
    extension.isInstalled = entry[isInstalled]
    sync.extension = extension
    return sync
}
