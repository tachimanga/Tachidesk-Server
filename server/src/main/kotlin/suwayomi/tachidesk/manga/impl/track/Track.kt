...
object Track {
    ...
    fun getTrackRecordsByMangaId(mangaId: Int): List<MangaTrackerDataClass> {
        ...
        return trackers
            ...
            .map {
                ...
                if (record != null) {
                    val track = Track.create(it.id).also { t ->
    ...
    suspend fun bind(input: TrackSearchDataClass) {
        ...
        if (track.started_reading_date <= 0) {
            val oldestChapter = transaction {
                ChapterTable.select { (ChapterTable.manga eq input.mangaId.toInt()) and (ChapterTable.isRead eq true) }
    ...
    private suspend fun trackChapter(mangaId: Int) {
        ...
        val chapterNumber = chapter?.get(ChapterTable.chapter_number)
        println("[Tracker]mangaId $mangaId chapter:${chapter?.get(ChapterTable.name)} chapterNumber:$chapterNumber")
        if (chapterNumber != null && chapterNumber > 0) {
    ...
    private suspend fun trackChapter(mangaId: Int, chapterNumber: Double) {
        if (!trackerManager.hasLoggedTracker()) {
            return
        }
        ...
        records.forEach {
            ...
            println("[Tracker]trackChapter id:${tracker?.id} login:$isLogin mangaId:$mangaId dbChapter:$lastChapterRead toChapter:$chapterNumber")
